---
title: "Over representation analysis (ORA) using R"
output:
        html_document:
            includes:
...
- An *input* list of differentially expressed (subject to some cutoff for significance) that will be queried.   
- A *reference/background gene set* of genes that are expected to be measured
...
In addition to the browser version of `WebGestalt`, the authors also created an [R package called `WebGestaltR`](https://bzhanglab.github.io/WebGestaltR/) that can be used to run the tool in R instead of by using their webserver. 
