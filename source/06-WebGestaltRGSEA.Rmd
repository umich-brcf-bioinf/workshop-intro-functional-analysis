...
But what about our input? Why did we use the fold-change column?

<!-- what to add here?-->

For more details, this [Pathways Common guide](https://www.pathwaycommons.org/guide/primers/data_analysis/gsea/) delves into the history and more details about the algorithm.

<!-- What about if we used a different ranking approach?

```{r isc_interest_set2}
# -------------------------------------------------------------------------
# Select gene and avg_log2FC columns
isc_d21_v_d7_gsea_2 = isc_d21_v_d7 %>%
  dplyr::mutate(rank = sign(avg_log2FC) * -log10(p_val)) %>%
  dplyr::select(gene, rank)

# -------------------------------------------------------------------------
# Preview the table
head(isc_d21_v_d7_gsea_2)
```
...
# -------------------------------------------------------------------------
# Run GSEA
isc_d21_v_d7_gsea_result2 = WebGestaltR(
    enrichMethod = 'GSEA',
    nThreads = 8,
    organism = 'mmusculus',
    enrichDatabase = c('pathway_KEGG'),
    interestGene = isc_d21_v_d7_gsea_2,
    interestGeneType = 'genesymbol',
    fdrThr = 0.1,
    outputDirectory = './results',
    projectName = 'D21_v_D7_GSEA_KEGG-alt', 
    cache = NULL)

# clean up session
gc()

isc_d21_v_d7_gsea_result2
```

If we incorporate p-values in the rankings, get generally similar results when it comes to the negative enrichments but lose the positive enrichment of ECM (which seems biologically relevant)

-->

# Results of GSEA

As with the ORA results, the GSEA method outputs similar results tables, gene identifier mappings, and HTML reports. 
...
Additionally, for each pathway with `FDR < fdrThr`, a GSEA enrichment plot is automatically generated. The following is an example:
