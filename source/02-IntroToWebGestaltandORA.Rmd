...
Behind the scenes, we used the same thresholds as the original analysis for the `log2FoldChange` and `pvalue` columns to identify DE gene. The we oua list of the IDs for the DE genes, using the `gene` column . In the next section, we re-create this file together but for simplicity we'll accept this as an input to use for our analysis.
