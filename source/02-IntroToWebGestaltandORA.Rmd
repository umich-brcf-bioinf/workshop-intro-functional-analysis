...
![WebGestalt homepage manual top]("images/content/02-webgestalt_TopOfManual.png")  
...
![WebGestalt homepage manual page 2]("images/content/02-webgestalt_Manual2ndPage.png")
...
```{r, echo=TRUE, message=FALSE, evaluate=FALSE}
# Repeat to generate background set (non-DE gene names)
deficient_nonDE <- deficient.v.control_bulkDE %>%
  filter(padj > 0.5 | abs(log2FoldChange) < log2(1.5))

deficient_nonDE_GeneList <- deficient_nonDE %>% pull(symbol) %>% na.omit()
```
...
<!-- planning to move this section to the start to give an overview of the results before using the tool -->
